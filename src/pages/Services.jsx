        ...
        <div className="px-gutter max-w-container-max mx-auto space-y-xl">
          {services.map((s, i) => (
            <Reveal key={s.slug} delay={i * 70}>
              <article className="grid lg:grid-cols-[160px_1fr_auto] gap-lg items-start bg-surface-container-lowest border border-outline-variant rounded-xl p-xl hover:border-primary transition-colors">
                <div>
                  <span className="text-[5rem] leading-none font-bold text-primary/15">
                    {s.code}
                  </span>
                </div>
                <div>
                ...
                <Link
                  to={`/services/${s.slug}`}
                  className="bg-primary-container text-on-primary px-lg py-md rounded-lg text-label-md font-semibold hover:bg-primary transition-all flex items-center gap-sm justify-self-start lg:justify-self-end"
                >
                  Explore <Icon name="east" className="!text-base" />
                </Link>
              </article>
            </Reveal>
          ))}
